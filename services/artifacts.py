...
def _important_page_lines(links: list[Any], *, limit: int = 16) -> list[str]:
    ...
    for item in links or []:
        ...
        if _is_unusable_important_page(
            ...
            continue
        # Prefer canonical prezzi over legacy EN alias if both appear.
        if href.rstrip("/").endswith("/pricing"):
            href = href.replace("/pricing", "/prezzi")
            if label.lower().startswith("404"):
                label = "Prezzi"
...
def fallback_llms_txt(url: str, scraped: dict[str, Any]) -> str:
    ...
    if headings:
        ...
        for heading in headings[:8]:
            ...
            h = h.replace(", sempre distinti.", " restano distinti.")
            if h:
                lines.append(f"- {h}")
        lines.append("")
    if important:
...
def _sanitize_generated_llms(content: str) -> str:
    ...
    for line in content.splitlines():
        low = line.lower()
        if re.search(r"404\s*not found|->\s*https?://[^\s]*pricing\b", low):
            if "pricing" in low and "prezzi" not in low:
                # Drop broken legacy pricing rows entirely.
                continue
