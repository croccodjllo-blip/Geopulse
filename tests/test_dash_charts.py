...
def test_signal_deck_is_wired_on_main_dashboard():
    ...
    assert "stroke-width: 0.95" in CSS.split("dash-area__line")[1][:120]
    assert "height: 10px" in CSS.split("dash-split__track")[1]
    assert "flex: 1 1 auto" in CSS.split("dash-area")[1]
    assert "dash-area__dates" in SIGNAL
    assert "0.52rem" in CSS.split("dash-area__x")[1][:160]
    assert 'preserveAspectRatio="xMidYMid meet"' in SIGNAL
...
def test_spark_and_delta_only_when_real_history():
    ...
    assert [t["label"] for t in dated["spark"]["ticks_x"]] == ["01/08", "", "21/08"]
